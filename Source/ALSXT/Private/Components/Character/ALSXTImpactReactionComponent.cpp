...
float UALSXTImpactReactionComponent::GetImpactFallenMinimumTime()
{
	return FGenericPlatformMath::Min(GetDynamicImpactFallenMinimumTime(), SelectImpactReactionSettings()->ImpactFallenMinimumTime);
}

float UALSXTImpactReactionComponent::GetAttackFallenMinimumTime()
{
...
void UALSXTImpactReactionComponent::StartImpactFallenTimer()
{
	float FallenMinTime = GetImpactFallenMinimumTime();
...
void UALSXTImpactReactionComponent::StartAttackFallenTimer()
{
	float FallenMinTime = GetAttackFallenMinimumTime();
	FALSXTImpactReactionState Params = GetImpactReactionState();
	Params.ImpactReactionParameters.FallenMinimumTime = FallenMinTime;
	Params.ImpactReactionParameters.CurrentFallenTime = FallenMinTime;
	SetImpactReactionState(Params);
			...
			for (FHitResult HitResult : HitResults)
			{
				...
				if (UKismetSystemLibrary::DoesImplementInterface(HitResult.GetActor(), UALSXTCollisionInterface::StaticClass()))
				{
					...
					if (UKismetSystemLibrary::CapsuleTraceSingleForObjects(GetWorld(), HitResult.ImpactPoint, StartLocation, CapsuleRadius, CapsuleHalfHeight / 2, ImpactReactionSettings.BumpTraceObjectTypes, false, IgnoreActorsOrigin, EDrawDebugTrace::None, OriginHitResult, false, FLinearColor::Green, FLinearColor::Red, 5.0f))
					{
						...
						{
							IALSXTCharacterInterface::Execute_CrowdNavigationReaction(HitResult.GetActor(), Character->GetDesiredGait(), ALSXTImpactSideTags::Right, ALSXTImpactFormTags::Blunt);
						}
...
void UALSXTImpactReactionComponent::StartBumpReaction(const FGameplayTag& Gait, const FGameplayTag& Side, const FGameplayTag& Form)
{
	...
	Montage = SelectedBumpReaction.Montage.Montage;
...
void UALSXTImpactReactionComponent::StartCrowdNavigationReaction(const FGameplayTag& Gait, const FGameplayTag& Side, const FGameplayTag& Form)
{
	...
	Montage = SelectedCrowdNavigationReaction.Montage.Montage;
	...
	USoundBase* Audio = GetImpactReactionSound(GetImpactReactionState().ImpactReactionParameters.CrowdNavigationHit).Sound.Sound;
	const auto StartYawAngle{ UE_REAL_TO_FLOAT(FRotator::NormalizeAxis(Character->GetActorRotation().Yaw)) };
...
void UALSXTImpactReactionComponent::StartAttackFallIdle(FAttackDoubleHitResult Hit)
{
	...
	if (Character->GetLocalRole() >= ROLE_Authority)
	{
		ServerStartAttackFallIdle(Montage);
	}
	else
	{
		Character->GetCharacterMovement()->FlushServerMoves();
		MulticastStartAttackFallIdle(Montage);
		// OnImpactReactionStarted(Hit.DoubleHitResult);
	}
}
...
void UALSXTImpactReactionComponent::ServerStartAttackFallIdle_Implementation(UAnimMontage* Montage)
{
	if (IsImpactReactionAllowedToStart(Montage))
	{
		MulticastStartAttackFallIdle(Montage);
...
bool UALSXTImpactReactionComponent::ServerStartAttackFallIdle_Validate(UAnimMontage* Montage)
{
	return true;
}

void UALSXTImpactReactionComponent::MulticastStartAttackFallIdle_Implementation(UAnimMontage* Montage)
{
	StartAttackFallIdleImplementation(Montage);
}
...
void UALSXTImpactReactionComponent::StartCrowdNavigationReactionImplementation(FActionMontageInfo Montage, USoundBase* Audio)
{
	if (IsCrowdNavigationReactionAllowedToStart(Montage.Montage))
	{
		...
		if (Audio)
		{
			...
			{
				UGameplayStatics::PlaySoundAtLocation(GetWorld(), Audio, CurrentImpactReactionState.ImpactReactionParameters.Hit.HitResult.HitResult.ImpactPoint,
					1.0f, 1.0f);
			}
			else
			{
				AudioComponent = UGameplayStatics::SpawnSoundAtLocation(GetWorld(), Audio, CurrentImpactReactionState.ImpactReactionParameters.Hit.HitResult.HitResult.ImpactPoint,
					NewRotation,
					1.0f, 1.0f);
			}
		}
		...
		Character->GetMesh()->AddImpulseToAllBodiesBelow(CurrentImpactReactionState.ImpactReactionParameters.Hit.HitResult.Impulse * 1000, CurrentImpactReactionState.ImpactReactionParameters.Hit.HitResult.HitResult.BoneName, false, true);
		...
		else
		{
			if (ShouldClutchImpactPoint())
			{
				ClutchImpactPoint(CurrentImpactReactionState.ImpactReactionParameters.Hit);
			}
		}
...
void UALSXTImpactReactionComponent::StartAttackFallIdleImplementation(UAnimMontage* Montage)
{
	if (IsImpactReactionAllowedToStart(Montage))
	{
